...
class BERTTrainer:
    ...
    def iteration(self, training_data, max_seq_len, train=True):
        ...
        for ori_seq_batch in pbar:
            ...
            ori_seq_batch = ori_seq_batch.to(self.device)       # [16, 512, 4]
            ...
            for b in range(batch):
                ...
                for i in mask80:
                    ...
                    input_ids[b][i] = mask_word #module.mask_word_np
                    loss_mask[b][i] = 1 
                for i in rand10:
                    rand_word = torch.tensor(self.midibert.get_rand_tok()).to(self.device)
                    input_ids[b][i] = rand_word #.module.get_rand_tok()
                    loss_mask[b][i] = 1 
                for i in cur10:
                    loss_mask[b][i] = 1 
            ...
            attn_mask = (input_ids[:, :, 0] != self.midibert.bar_pad_word).float().to(self.device)   # (4,512)
            ...
            outputs = torch.from_numpy(outputs).to(self.device) #to(device).long()   # (4,512)

            # accuracy
            #ori_seq_batch = ori_seq_batch.astype(float)
            #ori_seq_batch = torch.from_numpy(ori_seq_batch).long().cuda() #to(device).long()
