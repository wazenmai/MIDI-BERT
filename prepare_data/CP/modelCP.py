...
#        self.load_model()
    ...
    # padding
    ########################################
    def padding(self, data, max_lens):
    ...
    def prepare_data(self, midi_paths):
        ...
        for events in all_events:
            ...
            for note_tuple in events:
                nts = []
                for e in note_tuple:
                    print(e.name, e.value, self.event2word[e.name])
                    if e.value in self.event2word[e.name]:
                        e_text = '{} {}'.format(e.name, e.value)
                        nts.append(self.event2word[e.name][e_text])
                        print('valid')
                    else:
                        print('sth is wrong! {}'.format(e))
                words.append(nts)
                #print(e)
